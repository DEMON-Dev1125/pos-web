  ...
  return (
    <div className={props.classes}>
      ...
      <StockDialog
        ...
        handleConfirm={handleStockConfirm}
        data={stockProducts}
      />
      <CommentDialog
        open={commentOpen}
      ...
      <div className="takeAwayHeader">
        ...
        <p className={clsx("p-0 m-0 fs-14 fw-bold", classes.headerTitle)}>
          {isTakeAway ? "Takeaway" : "Delivery"}{" "}
          {pendingSales.name ? " - " + pendingSales.name : ""}
        </p>
        <IconButton
          aria-describedby={id}
          aria-label="delete"
        ...
        >
          <div className="mainPopover">
            ...
            <div
              className={"mainPopOneRow disabled"}
              onClick={() => setMergeWith(true)}
            >
              <div className={"firstSection"}>
            ...
            <div
              className="mainPopOneRow bt"
              onClick={() => setDeliveryOwned(true)}
            >
              <div className={"firstSection"}>
      ...
      {!customerDetail && (
        <div className={"customerHeader"} onClick={addCustomer}>
          ...
          <p className={clsx("p-0 m-0 fs-14 ", classes.addCustomer)}>
            Add customer
          </p>
        </div>
      )}
      {customerDetail && (
        <div
          className={clsx(
            removeCustomer ? "customer-left-side" : "customer-right-side",
            "transitionArea"
          )}
        >
          <Swipeable
          ...
          >
            <div
              className="customerHeader justify-between"
              onClick={viewCustomer}
            >
              <div className="flex flex-row">
                <div className="circleArea backcolor-pink">
                  <p className="p-0 m-0 fs-12 fw-bold color-pink">
                    {customerDetail.shortName}
                  </p>
                </div>
                <div className="nameArea">
                  <p className="m-0 fs-10 fw-bold color-light-black">
                    {customerDetail.name}
                  </p>
          ...
          <div className="removeArea">
            <IconButton
              aria-label="delete"
              className={classes.customerRemoveBtn}
              onClick={() => {
                setCustomeDetail(null);
        ...
        {products.map((item, index) => {
          if (item && item.name) {
            return (
              <div
                key={index}
                className="height-50 borderBottomLight overflowHidden"
              >
                <Swipeable
                  onSwipedLeft={() => {
                    handleLeft(index);
                  ...
                  className={clsx(
                    classes.swipeArea,
                    isLeft[index] ? "left-side" : "right-side"
                  )}
                >
                  <div className="productArea">
                    ...
                    >
                      <p className="m-0 fs-10 fw-bold color-light-black max-140">
                        {item["name"]}
                      </p>
                      {item.type === 4 &&
                        item["name"] !== "bread" &&
                        item["variant"] && (
                          <p className="pt-5 m-0 fs-8 color-light max-140">
                            {item["variant"]["name"]}
                          </p>
                        )}
                      {item.modifiers && (
                      ...
                      {item.comment && (
                        <p className="pt-5 m-0 fs-8 color-light f-italic max-140">
                          {item.comment}
                        </p>
                      )}
                    </div>
                    <div className="checkPrice">
                      <p className="p-0 m-0 fs-12 fw-bold color-light-black">
                        L£{" "}
                        {_numberWithCommas(
                          Math.round(20000 * Number(item.amount))
                        )}
                      </p>
                    </div>
                  </div>
      ...
      {!isZero && (
        <div className="discountSection">
          <div
            className="p-10 flex flex-row justify-between align-center"
            style={{ borderBottom: "1px solid #E8ECEF" }}
          >
            <div
              className="flex-row flex align-center cursor-pointer"
          ...
          <div className="flex-row flex justify-between p-10">
            <p className="p-0 m-0 fs-12 color-light-black">
              VAT (11% included)
            </p>
            ...
            <div>
              <Button
                ...
                className={clsx(
                  classes.button,
                  classes.width124,
                  isTakeAway ? classes.primaryBtn : classes.disabledBtn
                )}
        ...
        <div className="p-10 flex flex-row">
          ...
          <Button
            ...
            className={clsx(
              classes.button,
              classes.padding0,
              !isZero ? classes.width165 : classes.widthFull
            )}
            classes={{ contained: classes.contained }}
          ...
          >
            ...
            {!isZero && (
              <span className="fw-bold fs-16">
                {"L£ " + _numberWithCommas(getTotalPrice())}
              </span>
            )}{" "}
          </Button>
          {!isZero && (
            <div className={clsx("quantityArea", "backPrimary")}>
